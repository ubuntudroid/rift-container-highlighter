...
//!   - import paths rewritten to `crate::vendor::skylight`
//!   - `use nix::libc` dropped in favour of the `libc` crate directly
//!   - one `tracing::warn!` in the Drop impl replaced with `eprintln!`
//! Nothing else. Keep it that way so a re-diff stays readable.
...
use crate::vendor::skylight::cg_ok;
use crate::vendor::skylight::SLSSetWindowBackgroundBlurRadius;
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            if let Err(err) = cg_ok(SLSReleaseWindow(self.connection, self.id)) {
                // Deviation from upstream: `tracing::warn!`. Not worth a logging
                // framework for one line in a Drop impl.
                eprintln!("warning: failed to release CGS window {}: {err:?}", self.id);
            }
        }
    }
}
